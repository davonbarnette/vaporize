...
def addElements(img):
    ...
    allfiles = os.listdir("scripts/python_vaporize/elements/black/")
    ...
    for num in range(numElements):
        file_name = "scripts/python_vaporize/elements/black/ele_b"
...
def faceGlitch(img, face):
    ...
    for itr in range(0, int(numGlitches)):
        ...
        if dec % 2 == 0:
            ...
            img[face[1] + (itr * strp):face[1] + (itr * strp + strp), (face[0] + rng):rightExt] = img[face[1] + (
                        itr * strp):face[1] + (itr * strp + strp), face[0]:face[0] + face[2] - diff]
        else:
            backBound = face[0] - rng
            diff = 0
            # make corrections if glitch falls outside of image
            if backBound < 0:
                diff = abs(backBound)
                backBound = 0
            img[face[1] + (itr * strp):face[1] + (itr * strp + strp), (backBound):leftExt] = img[face[1] + (itr * strp):
                                                                                                 face[1] + (
                                                                                                             itr * strp + strp),
                                                                                             face[0]:face[0] + face[
                                                                                                 2] - diff]
...
def eyeDrag(img, eyes):
    ...
    for eye in eyes:
        ...
        for itr in range(0, numGlitches):
            ...
            img[line:line + drop, eye[0] + itr * strp:eye[0] + itr * strp + strp] = img[line, eye[0] + itr * strp:eye[
                                                                                                                      0] + itr * strp + strp]
...
def vaporize(img):
    ...
    face_cascade = cv2.CascadeClassifier('scripts/python_vaporize/cascade/haarcascade_frontalface_default.xml')
    eye_cascade = cv2.CascadeClassifier('scripts/python_vaporize/cascade/haarcascade_eye.xml')
